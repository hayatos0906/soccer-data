...
## 🚀 1. 開発の始め方（初期セットアップ）
1. 自分のアカウントで Supabase（無料）にプロジェクトを作成し、メンバー全員（先輩含む）を招待してください。おそらく私だけでいい。（slackのDMで送ります）
2. `backend/.env.example` をコピーして `backend/.env` を作成し、Supabaseの `SUPABASE_URL` と `SUPABASE_KEY` を書き込んでください。
   ```bash
   cp backend/.env.example backend/.env
   ```
3.あとはCursorでも、チャットAIでもいいので、このREADMEと backend/DESIGN_DOC.md を読み込ませて、以下の実装タスクを実行してください。
